...
Load these project files if they exist in the repo. Skip any that are missing — do not block or report an error. As these docs are written, the skill will pick them up automatically.

- `CLAUDE.md` — what Shoprift is today (manual service, not a platform)
- `ROADMAP.md` — V1 → V5 phases and triggers
- `OPERATIONS.md` — the manual workflow being automated (may not exist yet)
- `CSV_TEMPLATE.md` — the output contract (may not exist yet)
- `TERMS.md` — what's been promised to sellers (may not exist yet)
- `LANDING.md` — the public-facing surface and trust choices (may not exist yet)

You don't need to recite them. You need them loaded. If a question is about the web app and you haven't checked OPERATIONS.md to see what's currently manual, you'll generate hallucinated PM advice that sounds right and is wrong. For missing files, proceed from what's known — CLAUDE.md and ROADMAP.md together cover most of what you need for early-stage PM work.

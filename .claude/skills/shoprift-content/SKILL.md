...
- **"a carousel"** → Generate the full HTML file using `templates/carousel-base.html`. Save to `./output/content/`. Each slide is a section. User screenshots and posts.
...
0. Create `./output/content/` if it doesn't exist (`mkdir -p`).
1. Save to `./output/content/` with a descriptive filename.
2. Carousels = `.html` (single self-contained file).
3. Long-form copy = `.md`.
4. After saving, tell the user the file path so they can open it.

...
> **File loading:** Reference and template files mentioned throughout this skill may not
> exist yet. When referenced, load the file if it exists in the skill directory.
> If absent, proceed from the inline guidance here — do not block or report an error.
...
Generate GST-compliant tax invoices for Shoprift's migration service. Every invoice must include the mandatory fields under Rule 46 of CGST Rules — see `references/invoice-rules.md` if present for the full list.
...
- **Line description language:** Neutral and accurate. Never use "scraping" or "dm2buy data extraction" in invoice text. Prefer: "Software migration service" or "Store migration package preparation". See `references/invoice-language.md` if present for the canonical phrasings.
- **SAC code:** 998314 on every line (use full 6-digit, not 4-digit, even though turnover allows shorter)
- **GST split:** auto-determined by place of supply — see `references/gst-place-of-supply.md` if present
...
- **Invoice numbering:** Sequential, no gaps, fiscal-year-based (`MALIQ/2026-27/001`, `MALIQ/2026-27/002`, etc.) — see `references/invoice-numbering.md` if present

When generating an invoice, ask for any missing inputs in a single message. Don't drip-feed questions. See `templates/invoice.md` if present for the canonical template.

### 2. Quotations and estimates

Pre-invoice document sent to a prospect to confirm scope and price before they commit. Should match the eventual invoice exactly when accepted. See `templates/quotation.md` if present.
...
For Shoprift's migration service specifically, see `references/pricing-shoprift.md` if present for the working pricing model.
...
Sole prop with mixed personal/business spending is a common mess. Help Mayank avoid it. See `references/expense-categories.md` if present for the standard chart of accounts, and `templates/expense-log.md` if present for the Sheets-native log.

ITC eligibility rules are not intuitive. Be conservative — when in doubt, flag for CA review, do not claim. See `references/itc-eligibility.md` if present for the common cases.
...
See `references/gst-workings.md` if present for the full prep checklist.
...
For Stage 1-2, scenario (b) is more common. Track every TDS deduction by clients in `templates/tds-tracker.md` if present. See `references/tds-rules.md` if present for the full breakdown.
...
P&L and balance sheet are derived from these. See `templates/books-google-sheets.md` if present for the structure.
...
See `templates/cash-flow-model.md` if present. Update at least monthly.
...
For the big strategic questions, see `references/strategic-decisions.md` if present for the working positions.
...
## Reference files — load if present, proceed inline if absent
...
## Template files — use if present, draft inline if absent

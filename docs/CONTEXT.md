...
| D — Launch | Phase 5 | ✅ Complete | PostHog ✅, billing-update webhook ✅, webhook dedup ✅, domain ✅, email replaced ✅. Migration 005 + shopify app deploy → shoprift-8 both done. |
...
- **Supabase migration needed** — `ALTER TABLE payment_receipts ADD COLUMN drive_url TEXT;` required for `/redeliver` to save/return Drive URLs on new jobs.
...
### 1 — Supabase migration (1 min, manual)
Run in Supabase SQL Editor:
```sql
ALTER TABLE payment_receipts ADD COLUMN drive_url TEXT;
```

### 2 — Railway deployment for Telegram bot (24/7 uptime)
...
- Env vars: all from `.env` + `GOOGLE_OAUTH_*` + `GOOGLE_DRIVE_FOLDER_ID` + `SUPABASE_SERVICE_KEY`
- Estimate: 20–30 min (Playwright already solved in existing worker)

### 3 — Shopify app T8 manual QA (browser required)
...
| `/quote <url>` | Runs recon, calculates ₹500+₹10/product, outputs copyable client quote message |
| `/queue <url>` | Add to file-backed batch queue. Runs sequentially, auto-starts next on completion. |
| `/queue` | Show queue status (running/pending/last 5 done) |
| `/queue clear` | Remove all pending jobs |
| `/queue clear <n>` | Remove job at position n from pending queue |
| `/receipt "Name" url amount upi-ref` | PDF receipt → writes to Supabase `payment_receipts` (incl. drive_url) |
| `/redeliver <receipt-no>` | Supabase lookup by receipt_no → resend Drive URL + client forward message |
| `/export` | All payment_receipts → CSV file |
...
| Keyboard buttons | Recon, Extract, Receipt, Weekly, Monthly, History, Jobs, Queue, Export, Help, ✖ Close |

**Keyboard layout (6 rows):**
```
🔍 Recon     ▶️ Extract
🧾 Receipt   📊 Weekly
📅 Monthly   🗂 History
...
**Supabase tables:**
- `payment_receipts` — receipt_no, date, client_name, store_url, amount_inr, upi_ref, products, collections, images, drive_url (add column if not done)
- `import_jobs` — used by Railway worker for Shopify app

**Queue file:** `apps/telegram-bot/output/_queue.json` — statuses: pending / running / done / failed
...
- **Razorpay routes deleted** — `/api/payment/create` and `/api/payment/verify` deleted.
...
- **Payment ledger** — Supabase `payment_receipts` table. Written on every `/receipt`. Includes `drive_url` (requires schema migration). Queried by `/history`, `/report`, `/export`, `/redeliver`.
- **Bot extraction architecture** — all spawn logic in `runExtraction(url, chatId, opts)`. Queue passes `{ isQueue: true }` to trigger `_queueJobDone` + `processQueue` on completion.
